...
public partial class admin_tabletform : System.Web.UI.Page
{
    SqlDataReader dr;
        ...
        if (FileUpload1.HasFile)
        {
            if (FileUpload1.PostedFile.ContentType == "image/jpeg" || FileUpload1.PostedFile.ContentType == "image/png")
            {
               ...
               sql = "update regid set regid='" + strreg + "'";
                ...
                string cmdString = @"Insert tablet(regno,brand,modelcode,altname,pic,releasedate,formfactor,dimension,weight,batcap,removablebat,colors,SAR,screensize,touchscreen,touchscreentype,resolution,pixelperinch,dispcolor,processor,RAM,intstorage,expstorage,rearcam,flash,frontcam,os,osver,javasupport,browser,wifi,wifistnd,GPS,bluetooth,NFC,bluetoothprofiles,DLNA,wifidir,MHL,HDMI,headphone,FM,USB,noofsim,simtype,infra,twog,threeg,fourg,compass,proxi,accmtr,amplightsensor,gyroscope,baromtr,temp,rate,youtubelink,source,verified) values('" + strreg + "','" + ddlbrand.SelectedItem + "','" + txtmodel.Text + "','" + ddlbrand.SelectedItem +" "+ txtmodel.Text + "','../pictures/" + fileName1 + "','" + ddlmonth.SelectedItem + "&nbsp;"+ddlyear.SelectedItem+"','" + ddlff.SelectedItem + "','"+txtdimension.Text+"','"+txtweight.Text+"','"+txtbatcap.Text+"','"+ddlremovbat.SelectedItem+"','"+txtbcolr.Text+"','"+txtsar.Text+"','"+txtscrsize.Text+"','"+ddltch.SelectedItem+"','"+ddltchtype.SelectedItem+"','"+txtresoltn.Text+"','"+txtppi.Text+"','"+txtdispcolr.Text+"','"+txtprocessor.Text+"','"+ddlram.SelectedItem+"','"+txtintmem.Text+"','"+txtexpmem.Text+"','"+txtrcam.Text+"','"+ddlflash.SelectedItem+"','"+txtfcam.Text+"','"+ddlos.SelectedItem+"','"+txtosver.Text+"','"+ddljavasup.SelectedItem+"','"+txtbrow.Text+"','"+ddlwifi.SelectedItem+"','"+txtwifistrd.Text+"','"+ddlgps.SelectedItem+"','"+txtbluetooth.Text+"','"+ddlnfc.SelectedItem+"','"+txtbluetoothprofl.Text+"','"+ddldlna.SelectedItem+"','"+ddlwifidir.SelectedItem+"','"+ddlmhl.SelectedItem+"','"+ddlhdmi.SelectedItem+"','"+txthdphn.Text+"','"+ddlfm.SelectedItem+"','"+txtusb.Text+"','"+txtnoofsim.Text+"','"+ddlsim.SelectedItem+"','"+txtsiminfra.Text+"','"+txt2g.Text+"','"+txt3g.Text+"','"+txt4g.Text+"','"+ddlcompass.SelectedItem+"','"+ddlprox.SelectedItem+"','"+ddlacc.SelectedItem+"','"+ddlamp.SelectedItem+"','"+ddlgyr.SelectedItem+"','"+ddlbaro.SelectedItem+"','"+ddltemp.SelectedItem+"','"+txtrate.Text+"','"+txtyoutube.Text+"','"+txtsource.Text+"','yes')";
